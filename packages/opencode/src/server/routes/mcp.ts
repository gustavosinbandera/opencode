  ...
  new Hono()
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Execute a live MCP tool by fully qualified id from the worker-backed MCP registry.",
      ...
      async (c) => {
        ...
        const text = (result.content ?? [])
          ...
          .join("\n\n")
        return c.json({ output: text || JSON.stringify(result.structuredContent ?? result, null, 2) })
      },
    )
    .get(
      "/",
